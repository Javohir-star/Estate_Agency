...
for path in html_files:
    ...
    if '{% load static %}' not in updated:
        updated = re.sub(r'<html[^>]*>', lambda m: m.group(0) + '\n{% load static %}', updated, count=1)
    updated = re.sub(r"\{\%\s*static\s*\"([^\"]*)\"\s*\%\}", r"{% static '\1' %}", updated)
    updated = re.sub(r"\{\%\s*static\s*'([^']*)'\s*\%\}", lambda m: "{% static '%s' %}" % m.group(1), updated)
    updated = re.sub(r"\{\%\s*static\s*'(https?://[^']+)'\s*\%\}", lambda m: m.group(1), updated)
    updated = re.sub(r'\{\%\s*static\s*"(https?://[^"]+)"\s*\%\}', lambda m: m.group(1), updated)
